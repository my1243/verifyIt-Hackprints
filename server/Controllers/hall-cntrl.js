...
module.exports = {
    ...
    createHall: async (req, res) => {
        try {
            ...
            if (hall !== null) { return res.status(200).send("already exist hall:"); }
    ...
    updateHall: async (req, res) => {
        try {
            ...
            if (tmpHall !== null) { return res.status(403).send("already exist hall with hall-no :)" + req.body.hallNo); }
            tmpHall = null;
    ...
    allocateHall: async (req, res) => {
        try {
            ...
            //check valid branch
            if (req.body.startRollNo.substring(0, 2) !==
                req.body.endRollNo.substring(0, 2))
                return res.status(200).send("invalid branch for start-roll-no & end-roll-no");
            ...
            if (range <= 0) return res.status(200).send("end-roll-no must be greater than start-roll-no");
            if (range > hall.capacity) return res.status(200).send("#students are out of capacity:)");
            ...
            let students = await StudentCollection.find({ branch: branch._id.toString() });

            for (i = 0; i < students.length; i++) {
                let student = students[i];
                let rollNumber = parseInt(student.rollNo.substring(2));

                if (parseInt(req.body.startRollNo.substring(2)) <= rollNumber &&
    ...
    deAllocateHall: async (req, res) => {
        try {
            ...
            //allocate to stduents
            let students = await StudentCollection.find({ branch: assignedBranch._id.toString() });
            for (i = 0; i < students.length; i++) {
                let student = students[i];
                let rollNumber = parseInt(student.rollNo.substring(2));
