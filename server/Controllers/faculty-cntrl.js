...
module.exports = {
	createFaculty: async (req, res) => {
		try {
			...
			if (faculty !== null)
				return res.status(200).send("already exist faculty:)" + req.body.fId);
			...
			if (faculty !== null)
				return res
					.status(200)
					.send("already exist faculty with :)" + req.body.fShortName);
			...
			return res.status(200).send(savedFaculty);
		} catch (error) {
			console.log(error);
	...
	updateFaculty: async (req, res) => {
		try {
			let faculty = await FacultyCollection.findOne({ fId: req.body.fId });
			if (faculty !== null && faculty._id.toString() !== req.body._id)
				return res.status(200).send("already exist faculty:)" + req.body.fId);
	...
	getSpecificFaculty: async (req, res) => {
		try {
			let faculty = await FacultyCollection.findOne({ fId: req.body.fId });
			if (faculty === null)
				return res.status(200).send("faculty doesn't exist :)");
