...
module.exports = {
  createBranch: async (req, res) => {
    try {
      ...
      if (branch !== null) {
        return res.status(200).send("already exist branch:");
      }
  ...
  updateBranch: async (req, res) => {
    try {
      ...
      if (branch !== null && branch._id.toString() !== req.body._id) {
        return res.status(200).send("already exist branch:");
      }
  ...
  updateSubject: async (req, res) => {
    try {
      ...
      return res
        .status(403)
        .send("doesn't exist this " + req.body.subjectCode + " subject:)");
    } catch (error) {
      return res.status(401).send(error);
    }
  },
    ...
    try {
      ...
      for (i = 0; i < branch.subjects.length; i++) {
        if (branch.subjects[i].subjectCode === req.body.subjectCode) {
          ...
          return res
            .status(200)
            .send("delete subject : " + req.body.subjectCode);
        }
      }
      return res
        .status(403)
  ...
  getSpecificSubject: async (req, res) => {
    try {
      ...
      if (branch === null)
        return res
          .status(403)
          .send("doesn't exist " + req.body.branch + " branch ");

      for (i = 0; i < branch.subjects.length; i++) {
        if (branch.subjects[i]._id.toString() === req.body.subject.toString()) {
          return res.status(200).send(branch.subjects[i]);
        }
      }
