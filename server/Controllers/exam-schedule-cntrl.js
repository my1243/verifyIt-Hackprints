...
module.exports = {
    ...
    createExamSchedule: async (req, res) => {
        try {
            ...
            if (branch === null) { return res.status(403).send("doesn't exist branch:)"); }

            //faculty
            let faculty = await FacultyCollection.findOne({ fId: req.body.fid });
            if (faculty === null) { return res.status(403).send("doesn't exist faculty:)"); }

            //hallNo
            let hall = await HallCollection.findOne({ hallNo: req.body.hallNo });
            if (hall === null) { return res.status(403).send("doesn't exist hall:)"); }
            ...
            if (subject === null) { return res.status(403).send("doesn't exist subject:)"); }
            ...
            if (examSchedule !== null) { return res.status(200).send("exam-schedule already exist :)"); }
    ...
    updateExamSchedule: async (req, res) => {
        try {
            ...
            let examSchedule = await ExamScheduleCollection.findOne({ _id: req.body._id });

            //remove allocation-id if new comes
            let tmpBranch = await BranchCollection.findOne({ _id: examSchedule.branch.toString() });
            if (examSchedule.branch._id.toString() !== branch._id.toString()) {

                let remainAllotments = tmpBranch.allotments.filter((currentAllotment) => {
                    return currentAllotment.allotment.toString() !== examSchedule._id.toString()
                });
            ...
            if (examSchedule.faculty._id.toString() !== faculty._id.toString()) {
                let tmpFaculty = await FacultyCollection.findOne({ _id: examSchedule.faculty.toString() });

                let remainAllotments = tmpFaculty.allotments.filter((currentAllotment) => {
                    return currentAllotment.allotment.toString() !== examSchedule._id.toString()
                });
            ...
            if (examSchedule.hall._id.toString() !== hall._id.toString()) {
                let tmpHall = await HallCollection.findOne({ _id: examSchedule.hall.toString() });
    ...
    deleteExamSchedule: async (req, res) => {
        try {
            let examSchedule = await ExamScheduleCollection.deleteOne({ _id: req.body._id });
            return res.status(200).send(examSchedule);
    ...
    getExamSchedulesByBranch: async (req, res) => {
        try {
            ...
            let examSchedule = await ExamScheduleCollection.findOne({ branch: branch._id });
